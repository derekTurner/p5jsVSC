...
var delay =  0;      // delay per step
var arrayradius      // radius of sphere
var maxDelay = .050;  // max angle of phase delay seconds
...
function setup() {
  ...
  slider4.action = function(){
    this.update();
    delay = (map(this.xs,0,255,0,maxDelay));
    text("Delay: "+ delay + "s",20,15);  // degrees
    placeSources();
  };
}
...
function  Source(x,y,phase) { // object defined by constructor function
    ...
    this.update = function() { // method
        ...
        for (this.count = 0; this.count < this.rings; this.count ++){
          ...
          if (phaseValue >= 0){
            ellipse(this.xpos, this.ypos, ( wavelength * resX * phaseValue));
          }
        }
        pop();
    }
...
function placeSources(){// endfire line
    ...
    for (sourceCount = 0; sourceCount < maxArray; sourceCount ++){
        ...
        linePhase = (( sourceCount * delay * frequency)%1)-1 ;// 0 - 1
...
function keyPressed() {
  ...
  /* for endfire modCurve always false use delays
